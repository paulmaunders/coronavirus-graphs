...
previous_days <- 50
future_days <- 30
total_days <- previous_days + future_days + 1

# Set up X axis to the total days you wish you forecast
days <- -previous_days:future_days
...
forecast_case_day = min(which(predicted_cases > forecast_cases)) - previous_days - 1  # look up the first day in the model which exceeds forecast_cases
...
p <- ggplot(df, aes(days, cases)) + 
  ...
  annotate("text", x=1, y=0, vjust="top", label="Today", colour="grey20", angle=90, size=4, family = "Courier") +
  ...
  annotate("text", x=forecast_case_day + 1, y=forecast_cases/2, vjust="top", label=format(Sys.Date() + forecast_case_day,'%A %d %B %Y'), colour="grey20", angle=90, size=4, family = "Courier") +
  ...
  geom_point(aes(x=forecast_case_day, y=predicted_cases[forecast_case_day + previous_days + 1]), color="red") +  # mark this point on the exponential growth curve
  
  # Add titles
  labs(title=paste("UK Coronavirus Cases - Last",previous_days,"days"), subtitle=format(Sys.Date(),'%A %d %B %Y'), caption="Graph by @paulmaunders")
