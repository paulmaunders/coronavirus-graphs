...
p <- plot(i[(rows-last):(rows)], fit=fit.second, color="blue", border = "white") +
  ...
  # Add labels
  labs(title=paste("Hart Coronavirus Cases"), 
...
ggsave(plot=p, filename = paste("~/Desktop/local-cases-incidence-recent-", i[rows]$dates, ".png", sep=""), device="png", dpi=300, height = 15 , width = 15, units = "cm")
