...
govapi <- fromJSON("https://api.coronavirus.data.gov.uk/v1/data?filters=areaName=England&structure=%7B%22date%22:%22date%22,%22hospitalCases%22:%22hospitalCases%22%7D&format=json")
...
p <- ggplot(data, aes(x=date,y=hospitalCases)) + 
  ...
  # Add titles
  labs(title="England Coronavirus Hospital Patients", subtitle=format(update_date,'%A %d %B %Y'), caption="Graph by @paulmaunders using R and ggplot. Data from coronavirus.data.gov.uk")

# Print the graph in the plot view
print (p)

# Save the graph as a file on your Desktop in 15x15cm square format - 300 dpi for printing
ggsave(plot=p, filename = paste("~/Desktop/england-hospital-patients-smooth-recent-", update_date, ".png", sep=""), device="png", dpi=300, height = 9 , width = 16, units = "cm")
